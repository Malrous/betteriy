if IY_LOADED and not _G.IY_DEBUG == true then
    warn("BetterIY is already loaded!")
	notify('Error', 'BetterIY is already loaded!')
    return
end
...
if not game:IsLoaded() then
    ...
    notLoaded.Text = "BetterIY is waiting for the game to load"
    game.Loaded:Wait()
    notLoaded:Destroy()
end
...
Title.TextSize = 18
Title.Text = "BetterIY v" .. currentVersion
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- IY Reborn Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs[#CMDs + 1] = {NAME = 'olddex / odex', DESC = 'Opens Old DEX by Raspberry Pi'}
...
CMDs[#CMDs + 1] = {NAME = 'facebang', DESC = '|owo|'}
...
CMDs[#CMDs + 1] = {NAME = 'keepiy', DESC = 'Auto execute IYR when you teleport through servers'}
...
CMDs[#CMDs + 1] = {NAME = 'walkfling', DESC = 'Basically the fling command but uses your root instead of your character'}
...
addcmd('unctest',{'unc'},function(args, speaker)
	notify("Loading",'Hold on a sec')
	-- credits to NamingConvention, creator of old UNC Test
...
addcmd('unfacebang',{},function(args, speaker)
	bang:Stop()
	bangAnim:Destroy()
	bangDied:Disconnect()
	isbanging = false
...
addcmd('securedex',{'bypasseddex'},function(args, speaker)
	loadstring(game:HttpGet("https://raw.githubusercontent.com/Babyhamsta/RBLX_Scripts/main/Universal/BypassedDarkDexV3.lua", true))()
end)

addcmd('multipleiyinstances',{'multiiy'},function(args, speaker)
	multiiy = true
	while true do
		if multiiy == true then
	IY_LOADED = false
	_G.IY_DEBUG = true
		end
		wait()
...
addcmd('stopnotify',{'nonotify', 'antinotify'},function(args, speaker)
	nonotify = true
	wait(1)
	while true do
...
addcmd('allownotify',{'anotify'},function(args, speaker)
	nonotify = false
	notify("Nonotify",'Nonotify turned off')
end)
...
addcmd('addunc',{'moreunc'},function(args, speaker)
	notify("Loading",'Adding more UNC.. (THIS MAY REMOVE SOME UNC)')
	loadstring(game:HttpGet("https://github.com/FurryBoyYT/moreunc/raw/main/source"))({
		PrintLogs = true,
		OverrideExistingFunctions = false
	})
end)

addcmd('terminaldiscord',{'terminaldc'},function(args, speaker)
	if setclipboard then
	notify("Announcement",'We now partner with Terminal by nxtgen! Copied Terminals discord invite')
	setclipboard("https://discord.gg/RnFPnjyBRQ")
	end
end)
...
addcmd("walkfling", {}, function(args, speaker)
    ...
    repeat RunService.Heartbeat:Wait()
        ...
        vel = root.Velocity
        root.Velocity = vel * 100000000 + Vector3.new(0, 100000000, 0)
